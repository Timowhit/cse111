...
def test_pressure_loss_from_pipe():
    ...
    assert pressure_loss_from_pipe(0.286870, 1800.00, 0.013, 1.65) == approx(-110.837, abs=0.001)
...
def test_reynolds_number():
    assert reynolds_number(0.048692, 0.00) == approx(0.0, abs=1.0)
    assert reynolds_number(0.048692, 1.65) == approx(80069, abs=1.0)
    assert reynolds_number(0.048692, 1.75) == approx(84922, abs=1.0)
    assert reynolds_number(0.286870, 1.65) == approx(471729, abs=1.0)
    assert reynolds_number(0.286870, 1.75) == approx(500318, abs=1.0)
